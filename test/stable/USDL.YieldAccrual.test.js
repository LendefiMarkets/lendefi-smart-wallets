...
describe("USDL - Yield Accrual and Rebasing", function () {
    ...
    describe("Yield Accrual", function () {
        it("Should increase totalAssets after yield accrual", async function () {
            const { usdl, usdc, yieldVault, manager, depositAmount } = await loadFixture(setupWithDeposit);
            ...
            await usdl.connect(manager).accrueYield();
        ...
        it("Should increase rebase index after yield accrual", async function () {
            const { usdl, yieldVault, manager } = await loadFixture(setupWithDeposit);
        ...
        it("Should increase user balance after yield accrual", async function () {
            const { usdl, yieldVault, manager, user1 } = await loadFixture(setupWithDeposit);
        ...
        it("Should emit YieldAccrued event", async function () {
            ...
            await expect(usdl.connect(manager).accrueYield())
                .to.emit(usdl, "YieldAccrued");
        });
        ...
        it("Should harvest yield into USDC balance", async function () {
            ...
            // Verify USDC is in yield vault, not in contract
            ...
            // Should have harvested yield into USDC balance
            const expectedYield = depositAmount * (multiplier - 1000000n) / 1000000n;
            expect(await usdc.balanceOf(usdlAddress)).to.be.closeTo(expectedYield, 1);
        });
        ...
        it("Should revert if caller is not manager", async function () {
            ...
            await expect(
                usdl.connect(user1).accrueYield()
            ).to.be.reverted;
        });
    });
    ...
    describe("Rebasing Mechanics", function () {
        it("Should maintain correct share ratio after rebase", async function () {
            const { usdl, yieldVault, manager, user1, depositAmount } = await loadFixture(setupWithDeposit);
        ...
        it("Should distribute yield proportionally to all holders", async function () {
            const { usdl, usdc, yieldVault, manager, user1, user2 } = await loadFixture(setupWithDeposit);
            const usdlAddress = await usdl.getAddress();
        ...
        it("Should allow user to redeem full balanceOf after yield", async function () {
            const { usdl, usdc, yieldVault, manager, user1 } = await loadFixture(setupWithDeposit);
    ...
    describe("TotalAssets Tracking", function () {
        ...
        it("totalAssets should reflect yield after accrual", async function () {
            const { usdl, yieldVault, manager, depositAmount } = await loadFixture(setupWithDeposit);
    ...
    describe("Automation (Chainlink Keeper)", function () {
        it("checkUpkeep should return false before interval", async function () {
            const { usdl, yieldVault } = await loadFixture(setupWithDeposit);
            
            await yieldVault.setYieldMultiplier(ethers.parseUnits("1.1", 6));

            const [upkeepNeeded] = await usdl.checkUpkeep("0x");
            expect(upkeepNeeded).to.be.false;
        });

        it("checkUpkeep should return false without yield", async function () {
            const { usdl } = await loadFixture(setupWithDeposit);
        ...
        it("performUpkeep should revert when not needed", async function () {
            ...
            await expect(
                usdl.performUpkeep("0x")
            ).to.be.revertedWithCustomError(usdl, "UpkeepNotNeeded");
        });

        it("Should disable automation when interval is 0", async function () {
            const { usdl, yieldVault, owner } = await loadFixture(setupWithDeposit);
            
            await usdl.connect(owner).setYieldAccrualInterval(0);
    ...
    describe("Interval Configuration", function () {
        it("Should set yield accrual interval", async function () {
            ...
            await usdl.connect(owner).setYieldAccrualInterval(newInterval);

            expect(await usdl.yieldAccrualInterval()).to.equal(newInterval);
        });
        ...
        it("Should emit YieldAccrualIntervalUpdated event", async function () {
            ...
            await expect(usdl.connect(owner).setYieldAccrualInterval(newInterval))
                .to.emit(usdl, "YieldAccrualIntervalUpdated")
        ...
        it("Should revert if interval too short", async function () {
            ...
            await expect(
                usdl.connect(owner).setYieldAccrualInterval(10)
            ).to.be.revertedWithCustomError(usdl, "AutomationIntervalTooShort")
             .withArgs(10);
        });
        ...
        it("Should allow disabling with 0", async function () {
            ...
            await usdl.connect(owner).setYieldAccrualInterval(0);
            expect(await usdl.yieldAccrualInterval()).to.equal(0);
        });
        ...
        it("Should revert if not admin", async function () {
            ...
            await expect(
                usdl.connect(user1).setYieldAccrualInterval(7200)
            ).to.be.reverted;
        });
    });
});

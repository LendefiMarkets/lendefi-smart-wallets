...
describe("USDL - Yield Asset Management", function () {
    ...
    async function addSingleYieldAsset(usdl, manager, yieldVault, usdc) {
    ...
    describe("Add Yield Asset", function () {
        it("Should add yield asset with zero weight (inactive)", async function () {
            const { usdl, manager, yieldVault, usdc } = await loadFixture(usdlFixture);
            ...
            expect(await usdl.getYieldAssetWeight(await yieldVault.getAddress())).to.equal(0);
        });
        ...
        it("Should store correct yield asset config", async function () {
            ...
            const config = await usdl.getYieldAssetConfig(vaultAddress);
        ...
        it("Should emit YieldAssetAdded event", async function () {
            ...
            await expect(
                usdl.connect(manager).addYieldAsset(
                    vaultAddress,
                    await usdc.getAddress(),
                    vaultAddress,
                    ASSET_TYPE.ERC4626
                )
            ).to.emit(usdl, "YieldAssetAdded")
        ...
        it("Should revert if deposit token is zero address", async function () {
            const { usdl, manager, yieldVault } = await loadFixture(usdlFixture);
        ...
        it("Should revert if asset already exists", async function () {
            ...
            await expect(
                usdl.connect(manager).addYieldAsset(vaultAddress, usdcAddress, vaultAddress, ASSET_TYPE.ERC4626)
            ).to.be.revertedWithCustomError(usdl, "AssetAlreadyExists")
             .withArgs(vaultAddress);
        });

        it("Should revert if caller is not manager", async function () {
            const { usdl, user1, yieldVault, usdc } = await loadFixture(usdlFixture);
            
            await expect(
                usdl.connect(user1).addYieldAsset(
                    await yieldVault.getAddress(),
                    await usdc.getAddress(),
                    await yieldVault.getAddress(),
                    ASSET_TYPE.ERC4626
        ...
        it("Should add multiple yield assets", async function () {
            const { usdl, manager, yieldVault, yieldVault2, usdc } = await loadFixture(usdlFixture);
            const usdcAddress = await usdc.getAddress();
            ...
            expect(await usdl.getYieldAssetCount()).to.equal(2);
        });

        it("Should revert when max yield assets reached", async function () {
            const { usdl, manager, usdc } = await loadFixture(usdlFixture);
            const usdcAddress = await usdc.getAddress();
            ...
            ).to.be.revertedWithCustomError(usdl, "MaxYieldAssetsReached")
             .withArgs(10);
        });
    });
    ...
    describe("Update Weights", function () {
        it("Should update weight for single asset", async function () {
            ...
            expect(await usdl.getYieldAssetWeight(vaultAddress)).to.equal(10000);
        });
        ...
        it("Should update weights for multiple assets", async function () {
            ...
            await usdl.connect(manager).addYieldAsset(vault1Address, usdcAddress, vault1Address, ASSET_TYPE.ERC4626);
            await usdl.connect(manager).addYieldAsset(vault2Address, usdcAddress, vault2Address, ASSET_TYPE.ERC4626);
            await usdl.connect(manager).updateWeights([6000, 4000]);

            expect(await usdl.getYieldAssetWeight(vault1Address)).to.equal(6000);
            expect(await usdl.getYieldAssetWeight(vault2Address)).to.equal(4000);
        });
        ...
        it("Should emit YieldAssetWeightUpdated events", async function () {
            ...
            await expect(usdl.connect(manager).updateWeights([10000]))
                .to.emit(usdl, "YieldAssetWeightUpdated")
        ...
        it("Should revert if weights length mismatch", async function () {
            ...
            await expect(
                usdl.connect(manager).updateWeights([10000])
            ).to.be.revertedWithCustomError(usdl, "LengthMismatch")
             .withArgs(1, 2);
        });
        ...
        it("Should revert if total weight is not 10000", async function () {
            ...
            await expect(
                usdl.connect(manager).updateWeights([5000])
            ).to.be.revertedWithCustomError(usdl, "InvalidTotalWeight")
             .withArgs(5000);
        });

        it("Should revert if caller is not manager", async function () {
            const { usdl, user1, manager, yieldVault, usdc } = await loadFixture(usdlFixture);
            ...
            await expect(
                usdl.connect(user1).updateWeights([10000])
            ).to.be.reverted;
        });

        it("Should auto-drain asset when weight goes to zero", async function () {
            const { usdl, manager, yieldVault, yieldVault2, usdc, user1 } = await loadFixture(usdlFixture);
            const usdcAddress = await usdc.getAddress();
            const vault1Address = await yieldVault.getAddress();
            const vault2Address = await yieldVault2.getAddress();
            const usdlAddress = await usdl.getAddress();
    ...
    describe("Remove Yield Asset", function () {
        it("Should remove yield asset with zero weight and zero balance", async function () {
            ...
            await usdl.connect(manager).removeYieldAsset(vaultAddress);

            expect(await usdl.getYieldAssetCount()).to.equal(0);
        });
        ...
        it("Should emit YieldAssetRemoved event", async function () {
            ...
            await expect(usdl.connect(manager).removeYieldAsset(vaultAddress))
                .to.emit(usdl, "YieldAssetRemoved")
                .withArgs(vaultAddress);
        });
        ...
        it("Should revert if asset not found", async function () {
            ...
            await expect(
                usdl.connect(manager).removeYieldAsset(vaultAddress)
            ).to.be.revertedWithCustomError(usdl, "AssetNotFound")
             .withArgs(vaultAddress);
        });
        ...
        it("Should revert if asset still has weight", async function () {
            ...
            await expect(
                usdl.connect(manager).removeYieldAsset(vaultAddress)
            ).to.be.revertedWithCustomError(usdl, "AssetStillActive")
             .withArgs(vaultAddress, 10000);
        });
    });
    ...
    describe("View Functions", function () {
        it("Should return all yield assets with weights", async function () {
            ...
            const [tokens, weights] = await usdl.getAllYieldAssets();
        ...
        it("Should return zero weight for non-existent asset", async function () {
            ...
            expect(await usdl.getYieldAssetWeight(ethers.ZeroAddress)).to.equal(0);
        });
    });
});
